...
new CreatePDS().dataset("ADCDMST.DBBCBL.FL1").options("cyl space(1,1) lrecl(80) dsorg(PO) recfm(F,B) dsntype(library)  msg(1)").execute()

new CreatePDS().dataset("ADCDMST.DBBCBL.FL").options("cyl space(1,1) lrecl(80) dsorg(PO) recfm(F,B) dsntype(library) msg(1)").execute()

//***** copy zFS directory to a PDS *****//

def file = new File("/u/ADCDMST/dbbcbl/hello.cbl")

def copy = new CopyToPDS()

copy.setFile(file)

copy.setDataset("ADCDMST.DBBCBL.FL1")

copy.setMember("HELLO")

copy.execute()

new CopyToPDS().file(new File("/u/adcdmst/dbbcbl/hello.cbl")).dataset("ADCDMST.DBBCBL.FL1").member("HELLO").execute()

//****** cmpl the Program ******//

def cmpl = new MVSExec().pgm("IGYCRCTL").parm("LIB")

cmpl.dd(new DDStatement().name("SYSIN").dsn("ADCDMST.DBBCBL.FL1(HELLO)").options("shr"))

cmpl.dd(new DDStatement().name("SYSLIN").dsn("ADCDMST.DBBCBL.FL(HELLO)").options("shr"))
...
cmpl.copy(new CopyToHFS().ddName("SYSPRINT").file(new File("/u/adcdmst/dbbcbl/hello.log")))
